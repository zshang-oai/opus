...
func tocByte(config Configuration, code frameCode) byte {
	return byte(config<<3) | byte(code)
}
...
func TestParsePacketFramesRFC6716MalformedRules(t *testing.T) {
	...
	tocCode0 := tocByte(9, frameCodeOneFrame)
	tocCode1 := tocByte(9, frameCodeTwoEqualFrames)
	tocCode2 := tocByte(9, frameCodeTwoDifferentFrames)
	tocCode3 := tocByte(9, frameCodeArbitraryFrames)
...
func TestParsePacketFramesValidEdgeCases(t *testing.T) {
	...
	t.Run("code 2 allows two zero-length frames", func(t *testing.T) {
		...
		packet := []byte{tocByte(9, frameCodeTwoDifferentFrames), 0}
	...
	t.Run("code 3 CBR allows one zero-length frame", func(t *testing.T) {
		...
		packet := []byte{tocByte(9, frameCodeArbitraryFrames), 1}
		frames, err := parsePacketFrames(packet, tableOfContentsHeader(packet[0]))
	...
	t.Run("code 3 padding continuation bytes contribute 254 trailing bytes", func(t *testing.T) {
		...
		packet := []byte{tocByte(9, frameCodeArbitraryFrames), 0b01000001, 255, 0, 0xAA}
